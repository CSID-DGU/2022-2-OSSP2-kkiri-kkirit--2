...
class InfoeditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tv_name = findViewById<EditText>(R.id.edit_name)
        tv_age = findViewById<EditText>(R.id.edit_age)
        tv_pass = findViewById<EditText>(R.id.edit_pass)
                ...
                Response.Listener<String> { response ->
                    try {
                        ...
                        if (success) { // 업데이트에 성공한 경우
                            Toast.makeText(
                                getApplicationContext(), "회원 정보를 업데이트 하였습니다.",
                                Toast.LENGTH_SHORT
                            ).show()
